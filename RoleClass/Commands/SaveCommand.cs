...
namespace RoleClass.Commands
{
	class SaveCommand : ICommandHandler
	{
		...
		public string[] OnCall(ICommandSender sender, string[] args)
		{
			...
			if (args == null || args.Length <= 0) return new string[] { GetUsage() };
			string trueRankName = args[0].ToLower();
			// cleanup old files
			const string path = "rc-config.dat";
			const string path2 = "dictionary.bin";
			const string newPath = "roleclass.cfgbin";
			if (File.Exists(path))
				File.Delete(path);
			...
			if (args == null || args.Length <= 2) return new string[] { GetUsage() };
			...
			{
				using (FileStream fileSteam = File.Open(newPath, FileMode.OpenOrCreate))
				{
					formatter.Serialize(fileSteam, serializingData);
					return new string[] { "Saved configuration for " + trueRankName + ":" + trueClassName };
				}
			}
			using (FileStream fileStream = File.Open(newPath, FileMode.Append))
			{
